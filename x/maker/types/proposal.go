...
func validateBackingRiskParams(params *BackingRiskParams) error {
	...
	if params.MaxGridMint != nil && params.MaxGridMint.IsNegative() {
		return fmt.Errorf("max grid mint value must be not negative")
	}
...
func validateCollateralRiskParams(params *CollateralRiskParams) error {
	...
	if params.CatalyticIronRatio != nil && (params.CatalyticIronRatio.IsNegative() || params.CatalyticIronRatio.GT(sdk.OneDec())) {
		return fmt.Errorf("catalytic iron ratio must be in [0, 1]")
	}
